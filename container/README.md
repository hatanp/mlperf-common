...
- Open MPI and InfiniBand verbs userspace support;
...
The multi-stage build compiles `nccl-tests` and installs the
pinned NVIDIA `cuda.bindings` wheel used by `ncclstage`. The wheel and its
...
The final stage executes the CLI help path, imports the CUDA wrapper, loads
NCCL and prints its version, and reports regular files, total filesystem
entries, and bytes below `/opt/ncclstage`. These checks catch builder/runtime
Python mismatches without requiring a GPU. GPU, MPI, network, whole-image
...
Pinned dependency versions can be overridden deliberately with Docker build
...
framework, HPC-X, SHARP, or NCCL-plugin stack. A successful NCCL test establishes
the behavior of the NCCL and MPI packages in this image. It does not prove that
a larger training image has identical communication behavior or performance.
